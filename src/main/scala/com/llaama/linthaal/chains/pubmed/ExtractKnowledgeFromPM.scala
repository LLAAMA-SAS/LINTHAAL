...
import com.llaama.linthaal.agents.google.BioAgentDefault
...
import com.llaama.linthaal.chains.pubmed.ExtractKnowledgeFromPM.CmdAndResponses

/** Linthaal - info@llaama.com - August 2025
  */
object ExtractKnowledgeFromPM {
  ...
  def apply(): Behavior[Cmd] = {
    Behaviors
      .setup[CmdAndResponses] { ctx =>
        new ExtractKnowledgeFromPM(ctx).start()
      }
      .narrow
  }
}

private class ExtractKnowledgeFromPM(ctx: ActorContext[CmdAndResponses]) {
  import ExtractKnowledgeFromPM.*

  val aigen = BioAgentDefault()
  ...
  def buildUpAbstracts(
    ...
    Behaviors.receiveMessage { case SomePMAbstracts(absts) =>
      ...
      if (newSet.size >= nbrAbstracts)
        val resp = aigen.extractMostImportantInformationFromAbstracts(absts)
