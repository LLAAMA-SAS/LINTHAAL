...
final class EutilsCalls(config: EutilsCalls.EutilsConfig)(implicit as: ActorSystem[?]) {
  ...
  def retrievePubmedAbstracts(pmids: Set[Int]): Future[NodeSeq] = {
    val queryUri = baseUrl + eSummary + pmids.mkString(",") + config.apiKeyURl
    getRemote(queryUri)
  }

  def eFetchPubmed(pmids: Set[Int]): Future[NodeSeq] = {
...
/** The current E-utilities end-points, mostly copied from
  * https://www.ncbi.nlm.nih.gov/books/NBK25497/
  *
  * EInfo (database statistics) utils.ncbi.nlm.nih.gov/entrez/eutils/einfo.fcgi
  *
  * Provides the number of records indexed in each field of a given database, the date of the last
  * update of the database, and the available links from the database to other Entrez databases.
  *
  * ESearch (text searches) eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi
  *
  * Responds to a text question with the list of matching UIDs in a given database (for later use in
  * ESummary, EFetch or ELink), along with the term translations of the question.
  *
  ...
  * with a question key and web environment for the uploaded dataset.
  *
  ...
  *
  * Responds to a list of UIDs in a given database with either a list of related UIDs (and relevancy
  * scores) in the same database or a list of linked UIDs in another Entrez database; checks for the
  * existence of a specified link from a list of one or more UIDs; creates a hyperlink to the
  * primary LinkOut provider for a specific UID and database, or lists LinkOut URLs and attributes
  * for multiple UIDs.
  *
  * EGQuery (global question) eutils.ncbi.nlm.nih.gov/entrez/eutils/egquery.fcgi
  *
  * Responds to a text question with the number of records matching the question in each Entrez
  * database.
  *
  ...
  *
  * NCBI Entrez Databases Entrez Database UID common name E-utility Database Name BioProject
  * BioProject ID bioproject BioSample BioSample ID biosample Books Book ID books Conserved Domains
  * PSSM-ID cdd dbGaP dbGaP ID gap dbVar dbVar ID dbvar Gene Gene ID gene Genome Genome ID genome
  * GEO Datasets GDS ID gds GEO Profiles GEO ID geoprofiles HomoloGene HomoloGene ID homologene MeSH
  * MeSH ID mesh NCBI C++ Toolkit Toolkit ID toolkit NLM Catalog NLM Catalog ID nlmcatalog
  * Nucleotide GI number nuccore PopSet PopSet ID popset Probe Probe ID probe Protein GI number
  * protein Protein Clusters Protein Cluster ID proteinclusters PubChem BioAssay AID pcassay PubChem
  * Compound CID pccompound PubChem Substance SID pcsubstance PubMed PMID pubmed PubMed Central
  * PMCID pmc SNP rs number snp SRA SRA ID sra Structure MMDB-ID structure Taxonomy TaxID taxonomy
  */
