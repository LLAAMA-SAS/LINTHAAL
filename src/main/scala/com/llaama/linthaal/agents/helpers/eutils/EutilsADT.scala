...
object EutilsADT {
  ...
  def pmIdsFromXml(ns: NodeSeq): PMIdSearchResults = {
    ...
    val ids: Set[Int] = (ns \\ "eSearchResult" \\ "IdList" \ "Id").map(id => id.text.toInt).toSet

    val trans: Set[QueryTranslation] =
  ...
  def pmAbstractsFromXml(ns: NodeSeq): Set[PMAbstract] = {
  ...
  private def pmXmlAbstToText(n: NodeSeq): String = {
    (n \\ "AbstractText")
      ...
      .map(n =>
        if (n._1.nonEmpty && n._1 == "CONCLUSIONS") n._1 + ": " + n._2
        else n._2) // for now, only keep conclusions if there are many fields
      .mkString("\n")
  }
}
