...
import org.linthaal.core.SGMaterialization.WrapGenericFeedback
...
/**
 * input data for the worker as a map.
 * The input data can be provided as chunks.
 * @param data
 * @param dataStep
 */
case class AddWorkerData(data: Map[String, String], dataStep: DataLoad = DataLoad.Last) extends WorkerCommand
...
/**
 * The transitions to be applied once the worker is completed.
 * As default it will return all transitions, but it's possible for the worker to select transitions
 * to be applied based on the results produced (like a decision on which branch to follow up).
 * @param blueprintTransition
 * @param replyTo
 */
case class GetWorkerTransitions(blueprintTransition: List[BlueprintTransition], replyTo: ActorRef[ChosenTransitions]) extends WorkerCommand

/**
 * A worker can decide to start itself when it has all the input data it needs.
 * However it can also be asked to start. It's up to the worker to decide what prevails.
 * @param replyTo
 */
...
/**
 * what are the transitions to be triggered after this worker has been completed.
 * if nothing is returned, it will assume all.
 * @param transitions
 */
case class ChosenTransitions(transitions: List[BlueprintTransition]) extends WorkerResp

/**
 * A worker can be in different states
 * 1. it expects data Input
 * 2. once all data is available, it decides itself when it's time to start working, or it's asked to start
 * 3. Eventually, it will either succeed, succeed partially or fail.
 *
