...
import org.linthaal.core.adt.DataLoad.*
...
case class AddWorkerData(data: Map[String, String],
                         dataStep: DataLoad = Last, replyTo: ActorRef[WorkerState]) extends WorkerMsg
...
case class GetWorkerTransitions(blueprintTransition: List[BlueprintTransition],
                                replyTo: ActorRef[ChosenTransitions]) extends WorkerMsg

case object StopWorker extends WorkerMsg

case class WorkerResults(results: Map[String, String])

case class WorkerState(state: WorkerStateType= WorkerStateType.DataInput, 
                       percentCompleted: Int = 0, msg: String = "")

case class ChosenTransitions(transitions: List[BlueprintTransition])


enum WorkerStateType:
  case DataInput, WrongData, Running, Completed, Failed, SeemsInLimbo
