...
final class EutilsCalls(config: EutilsCalls.EutilsConfig)(implicit as: ActorSystem[_]) {
  ...
  private def getRemote(queryUri: String): Future[NodeSeq] = {
    ...
    val httpReq = HttpRequest(method = HttpMethods.GET, queryUri, entity = HttpEntity(ContentTypes.`text/xml(UTF-8)`, ""), protocol = HttpProtocols.`HTTP/1.1`)
...
/** The current E-utilities end-points, mostly copied from
  ...
  * the date of the last update of the database, and the available links from
  * the database to other Entrez databases.
  *
  * ESearch (text searches) eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi
  *
  * Responds to a text question with the list of matching UIDs in a given
  ...
  * Server, and responds with a question key and web environment for the
  * uploaded dataset.
  *
  ...
  * list of one or more UIDs; creates a hyperlink to the primary LinkOut
  ...
  *
  * Responds to a text question with the number of records matching the question
  ...
  * strings.
  *
  * NCBI Entrez Databases Entrez Database UID common name E-utility Database
  * Name BioProject BioProject ID bioproject BioSample BioSample ID biosample
  * Books Book ID books Conserved Domains PSSM-ID cdd dbGaP dbGaP ID gap dbVar
  * dbVar ID dbvar Gene Gene ID gene Genome Genome ID genome GEO Datasets GDS ID
  * gds GEO Profiles GEO ID geoprofiles HomoloGene HomoloGene ID homologene MeSH
  * MeSH ID mesh NCBI C++ Toolkit Toolkit ID toolkit NLM Catalog NLM Catalog ID
  * nlmcatalog Nucleotide GI number nuccore PopSet PopSet ID popset Probe Probe
  * ID probe Protein GI number protein Protein Clusters Protein Cluster ID
  * proteinclusters PubChem BioAssay AID pcassay PubChem Compound CID pccompound
  * PubChem Substance SID pcsubstance PubMed PMID pubmed PubMed Central PMCID
  * pmc SNP rs number snp SRA SRA ID sra Structure MMDB-ID structure Taxonomy
  * TaxID taxonomy
  */
