...
object SimplePromptJsonProt extends DefaultJsonProtocol {
  ...
  implicit object JsonChoice extends RootJsonFormat[Choice] {
    def write(c: Choice) = JsObject(
      "index" -> JsNumber(c.index),
      "message" -> c.message.toJson,
      "finish_reason" -> JsString(c.finishReason)
    )
  ...
  implicit object JsonChatResponse extends RootJsonFormat[ChatResponse] {
    override def write(obj: ChatResponse): JsValue = JsObject(
      "id" -> JsString(obj.id),
    ...
    override def read(json: JsValue): ChatResponse = {
      json.asJsObject.getFields("choices", "created", "id", "model", "object", "usage") match {
        case Seq(choices, JsNumber(created), JsString(id), JsString(model),
        JsString(obj), usage ) =>
          ChatResponse(id, chatObject = obj, created.toLong, model, usage.convertTo[Usage],
            choices = choices.convertTo[Seq[Choice]])
